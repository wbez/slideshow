...
View the [sample copy spreadsheet](https://docs.google.com/spreadsheet/pub?key=1GQmb_ipmrK4Y1qf2SG5HXKqeVv-Zbn1jPimOl6YLoOI#gid=0).
...
def read_my_google_doc():
    ...
    get_document('1GQmb_ipmrK4Y1qf2SG5HXKqeVv-Zbn1jPimOl6YLoOI', file_path)
    data = Copy(file_path)
...
```
* * * * * ubuntu bash /home/ubuntu/apps/slideshow/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test
```
...
|--------|------|-----|-----|
|slideshow|tweet|`location`||
|slideshow|facebook|`location`||
|slideshow|email|`location`||
|slideshow|new-comment||
|slideshow|open-share-discuss||
|slideshow|close-share-discuss||
|slideshow|summary-copied||
|slideshow|featured-tweet-action|`action`|
|slideshow|featured-facebook-action|`action`|
